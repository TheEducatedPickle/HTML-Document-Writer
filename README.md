...
The input component is rendered in a similar fashion, because it mimics an HTML document by design. It is essentially an HTML list of InputElements that pulls styling attributes from tree properties. It traverses the tree by depth first, and maps each HTMLElement to an InputElement. Each InputElement contains subcomponents, such as text fields and selectors, that are responsible for sending data to App.js to modify the HTMLElement it is mapped to.